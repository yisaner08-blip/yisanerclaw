...
### 注释规范
- 🚨 **必须** 每个函数、类、变量的声明处必须有中文注释说明用途
- 🚨 **必须** 关键逻辑、分支判断、复杂表达式必须有中文行内注释
- 模块级 docstring 说明用途
...
### 代码简洁性
- 🚨 **必须** 能用一行代码完成就不用两行，避免不必要的中间变量
- 保持代码紧凑但可读，不为了"清晰"而过度展开

### 可靠性
- 🚨 **必须** 回答基于可靠信息（文档、代码、实测结果）
- ⚠️ 不确定时明确说明"未验证/不确定"，并列出验证方法
- 不做没有依据的推测
